...
uniform highp float wf_mul;
uniform highp float Evapor;
...
void main(void)
{
 ...
 highp vec4 tmp = Dist1 + Dist2;
 highp float wf = f0 + tmp.x + tmp.y + tmp.z + tmp.w;
 
 wf *= wf_mul;
 wf = max(wf - Evapor, 0.0);
 highp float seep = clamp(ws / 15.0, 0.0, max(1.0 - wf, 0.0));
 wf += seep;
...
bool inRect(float x1,float x2, float y1, float y2){
    if(textureCoordinate.x>x1 && textureCoordinate.x<x2 && textureCoordinate.y>y1 && textureCoordinate.y<y2) 
